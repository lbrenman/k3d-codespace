...
JOB_POD=$(kubectl get pods -n lab7 -l job-name=pi-calculator -o jsonpath='{.items[0].metadata.name}')
kubectl logs $JOB_POD -n lab7
# You should see 2000 digits of pi

# Inspect the Job status
kubectl describe job pi-calculator -n lab7
...
kubectl get jobs -n lab7

# Read logs from the most recent CronJob pod
CRON_POD=$(kubectl get pods -n lab7 -l app.kubernetes.io/name=logger \
  --sort-by=.metadata.creationTimestamp \
  -o jsonpath='{.items[-1].metadata.name}' 2>/dev/null || \
  kubectl get pods -n lab7 --sort-by=.metadata.creationTimestamp \
  -o jsonpath='{.items[-1].metadata.name}')
kubectl logs $CRON_POD -n lab7
...
kubectl get pods -n lab7 -w
# Press Ctrl+C once the manual job completes

kubectl logs -n lab7 -l job-name=logger-manual

# ── Step 10: Suspend the CronJob ──────────────────────────────────────────────
# Suspend stops future scheduled runs without deleting anything
kubectl patch cronjob logger -n lab7 -p '{"spec":{"suspend":true}}'
kubectl get cronjob -n lab7
# SUSPEND column will show True

# Resume it
kubectl patch cronjob logger -n lab7 -p '{"spec":{"suspend":false}}'
