...
kubectl get secret mysql-credentials -n wordpress
...
MYSQL_POD=$(kubectl get pod -n wordpress -l app=mysql -o jsonpath='{.items[0].metadata.name}')

kubectl exec -it $MYSQL_POD -n wordpress -- \
